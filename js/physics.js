...
export const CONFIG = {
  ...
  releaseTime: 0.45,  // s of field-off on a retarget() — a field handoff, not a laser shot
...
let meltT = CONFIG.meltTime;          // duration of the melt/release we are in
let pending = null, hasPending = false; // formation to install when the release ends
let isWord = true;                     // is the live formation the sampled word?
let held = false;                      // holdRelease(): λ is owned by the renderer's morph
...
// Install a formation: `pts` = [[x, y], …] in physics-box coords, or null to sample the word.
function applyTargets(pts) {
  isWord = pts === null;
  const g = isWord ? sampleWord() : pts;
  const cap = Math.floor(N * 0.88);        // ~12% of particles stay ambient gas
  while (g.length > cap) g.splice((Math.random() * g.length) | 0, 1);
  hasT.fill(0);
  // greedy nearest-free-particle per site; O(T*N) once per formation, fine at this scale
...
export function resize(w, h) {
  ...
  buildHash();
  // the word re-samples itself at the new size; every other formation is authored by the
  // page, which re-calls retarget() with regenerated points after its own resize debounce.
...
export function step(dt) {
  ...
  if (mode === 'gas') {
    ...
    if (modeT > meltT) {
      // the field is off and the beads are in flight — this is the only moment a formation
      // may be swapped without a well popping into existence under a settled swarm
      if (hasPending) { applyTargets(pending); pending = null; hasPending = false; }
      mode = 'ramp'; modeT = 0; rampT = CONFIG.rampTime2; lambda = 0;
    }
  } else {
    lambda = 1;                  // 'text'
  }
...
export function melt() {
  ...
  meltL0 = lambda; meltT = CONFIG.meltTime;
  mode = 'melt'; modeT = 0;
  pulse = CONFIG.pulseKT;          // laser pulse-heating, and λ collapses -> melts
}

// Swap the formation. `pts` = [[x, y], …] in physics-box coords (see formations.js), or
// null to re-sample the word. The handoff is physics, never a tween: if the field is still
// up we first RELEASE it (λ decays with meltTau, no heat pulse — a field change is not a
// laser shot), let the beads fly ballistically, and only then ramp the new wells in. So the
// sheet visibly relaxes flat before the next formation's wells emerge; nothing pops.
//
// resize(): the word re-samples itself, but every other formation is generated by the page
// against the old viewport — so after a resize the PAGE must re-call retarget() with points
// regenerated for the new innerWidth/innerHeight.
// Release the field and KEEP it released — no re-ramp, no heat pulse. This is retarget()'s
// release half with the timer taken off: the melt runs with meltT = Infinity, so λ decays to
// 0 and stays there for as long as the hold is on. holdRelease(false) hands λ back with a
// fresh ramp, so the formation re-crystallises exactly as it does after a melt.
// Used by scene3d's board morph, which owns λ while it is scrubbing.
export function holdRelease(on) {
  on = !!on;
  if (on === held) return;
  held = on;
  if (on) { meltL0 = lambda; meltT = Infinity; mode = 'melt'; modeT = 0; }
  else { mode = 'ramp'; modeT = 0; rampT = CONFIG.rampTime2; lambda = 0; }
}

export function retarget(pts) {
  if (held) { applyTargets(pts); return; }   // the hold owns λ: install the sites, stay released
  if (lambda > 0.15) {
    pending = pts; hasPending = true;
    meltL0 = lambda; meltT = CONFIG.releaseTime;
    mode = 'melt'; modeT = 0;
  } else {                          // already released (or still gas): straight to the ramp
    applyTargets(pts);
    mode = 'ramp'; modeT = 0; rampT = CONFIG.rampTime2;
  }
}
